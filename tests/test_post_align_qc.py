...
class TestCellSampleData:
    """Tests for building the per-cell QC table."""

    def test_get_cell_samp_dat_builds_expected_columns(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            ...
            lambda outs_dir: pd.DataFrame(
                {"bc": ["AAAC", "TTGG"], "total_reads": [10000, 8000]}
            ),
        )

        samp_dat_df = get_cell_samp_dat(
            loaded_library,
        ...
        assert samp_dat_df["exclude"].tolist() == ["No", "YES"]
        assert samp_dat_df["exclude2"].tolist() == ["No", "YES"]
        assert samp_dat_df["cell_member"].tolist() == ["load_AAAC", "load_TTGG"]


class TestTotalReads:
    """Tests for total read extraction."""

    def test_get_total_reads_uses_per_barcode_metrics_when_available(self, tmp_path):
...
class TestSummaryStats:
    """Tests for library summary metrics."""

    def test_write_summary_stats_extracts_library_metrics(self, tmp_path):
        ...
        summary_df = write_summary_stats(samp_dat_df, library_row)
...
class TestLoadData:
    """Tests for loading Cell Ranger filtered feature-barcode data."""

    def test_load_data_filters_multiome_gene_expression_and_disambiguates_duplicates(
        self, tmp_path
    ):
